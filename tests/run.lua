...
local helpLine = runtime:match("commands: /dyni[^\n]*")
assertTruthy(helpLine and helpLine:find("/dyni selftest", 1, true), "trimmed command help still advertises selftest")
assertEqual(helpLine and helpLine:find("/dyni diag", 1, true) or nil, nil, "trimmed command help no longer advertises diag")
assertTruthy(helpLine and helpLine:find("full list: README", 1, true), "trimmed command help points at README for the full list")
assertTruthy(runtime:find("command == \"diag\"", 1, true), "runtime keeps /dyni diag wired")
assertTruthy(runtime:find("command == \"status\"", 1, true), "runtime keeps /dyni status wired")
...
assertEqual(Core.GetRowStatusText({ statusText = "кандидат" }, "ruRU"), "кандидат", "russian legacy candidate round-trips")
assertEqual(Core.GetRowStatusText({ statusText = "отправлено" }, "ruRU"), "отправлено", "russian legacy sent round-trips")
assertEqual(Core.GetRowStatusText({ statusText = "авто отправлено" }, "ruRU"), "авто отправлено", "russian legacy auto sent round-trips")
assertEqual(Core.GetRowStatusText({ statusText = "отправка" }, "ruRU"), "отправка", "russian legacy sending round-trips")
assertEqual(Core.GetRowStatusText({ statusText = "авто-отправка" }, "ruRU"), "авто-отправка", "russian legacy auto sending round-trips")
assertEqual(Core.GetRowStatusText({ statusText = "виспер не отправлен" }, "ruRU"), "виспер не отправлен", "russian legacy whisper failure round-trips")
assertEqual(Core.GetRowStatusText({ statusText = "тест" }, "ruRU"), "тест", "russian legacy test row round-trips")
assertEqual(Core.GetRowStatusText({ statusText = "Отпр." }, "ruRU"), "отправлено", "russian legacy short sent maps to sent")
assertEqual(Core.GetRowStatusText({ statusText = "Отпр..." }, "ruRU"), "отправка", "russian legacy short sending maps to sending")
assertEqual(Core.GetTextGlyphRequirement("플레이어"), "HANGUL", "hangul text requests a hangul glyph fallback")
assertEqual(Core.GetTextGlyphRequirement(string.char(225, 132, 128) .. "player"), "HANGUL", "hangul jamo lead requests a hangul glyph fallback")
assertEqual(Core.GetTextGlyphRequirement(string.char(225, 135, 191)), "HANGUL", "hangul jamo range end requests a hangul glyph fallback")
assertEqual(Core.GetTextGlyphRequirement("ab" .. string.char(225, 128, 128) .. "cd"), nil, "myanmar lead sharing E1 does not request hangul")
assertEqual(Core.GetTextGlyphRequirement(string.char(225, 136, 128) .. "player"), nil, "ethiopic lead sharing E1 does not request hangul")
